...
)  # Fichier trop lourd pour être ajouté sur le repo Git
...
df_all_data = df_all_data.drop(
    df_all_data[df_all_data["STATFOR Market Segment"] == "All-Cargo"].index
)

# On enlève les jets privés
df_all_data = df_all_data.drop(
    df_all_data[df_all_data["STATFOR Market Segment"] == "Charter"].index
)
df_all_data = df_all_data.drop(
    df_all_data[df_all_data["STATFOR Market Segment"] == "Business Aviation"].index
)
...
df_flights_emissions = df_all_data.drop(
    df_all_data[df_all_data["Emissions_kgCO2eq"] == 0].index
)
