...
index2 = [
    ...
    "CO2 vols avions (kt)",
    "CO2 construction nouveaux avions (kt)",
    "CO2 train (kt)",
    "Delta CO2",
]
...
with pd.ExcelWriter("resultats/resultats_par_trajet.xlsx") as writer:
    ...
    for j in range(len(numero_trajet)):
        ...
        for i in range(len(scenarii)):
            ...
            if i == 0:
                df_trajet_general["Initial"] = [
                    nb_passagers_avions_debut / 1000,
                    0,
                    CO2_debut / 1000,
                    0,
                    0,
            ...
            df_trajet_general[columns[i + 1]] = [
                nb_passagers_avions_fin / 1000,
                passagers_train / 1000,
                CO2_avions / 1000,
                CO2_nouv_avions / 1000,
                CO2_trains / 1000,
                delta_CO2 / 1000,
            ]
            df_trajet_par_avion[columns[i + 1]] = pass_fin

        df_trajet_par_avion.to_excel(
            writer, sheet_name="trajet" + str(numero_trajet[j])
        )
        df_trajet_general.to_excel(
            writer, sheet_name="trajet" + str(numero_trajet[j]), startcol=10
        )
