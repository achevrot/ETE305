...
### Fonction objectif
On veut minimiser, pour chaque couple de ville : $( \sum_j {CO_2}_j \times$ `nb_avions[j]` $+ \sum_j {CO_2}{train} \times ( passagers_{init} -\sum_j$ `nb_passagers[j]` $) + \sum_j {{CO_2}_{constr}}_j\times$ `nb_nouv_avions[j]`$) /1000$
